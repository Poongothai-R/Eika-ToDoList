...
// Please rename the files to .jsx so i know they are React components
//
function App() {
  // Comments -1 this initial comment is not necesary, as by the order of the variables in relation to the rest of the code, we know is the initialization code
  ...
  useEffect(() => {
    let myLocalCompleteList = JSON.parse(
      window.localStorage.getItem("myCompleteList")
    );
    ...
    <div className="App">
      ...
      <div>
        {ToDoList === null && anotherTask === 0 ? (
          ...
          <div>
            {" "}
            <GetItemInput checkData={task} AddItem={AddItemHandler} />
          </div>
        ) : (
          <div>
            {" "}
