...
function ViewList(props) {
  ...
  const sortDataHandle = (e) => {
    ...
    switch (sortObj.sortOrder) {
      case "asc":
        data.sort((a, b) =>
          a[sortObj.sortBy] > b[sortObj.sortBy]
            ? 1
            : a[sortObj.sortBy] < b[sortObj.sortBy]
            ? -1
            : 0
    ...
    return (
      <div key={index} className="view_item_card">
        ...
        <div className="view_item_center">
          <label>
            <span> {recs.name.toUpperCase()}</span> ,{" "}
            <span>{recs.price + " kr"}</span>
          </label>
        </div>
        <div className="view_item_right">
          ...
          <label htmlFor={"upload-img-".concat(`${index}`)}>
            {recs.preview ? (
              <img
                className="itemImage"
                src={recs.preview.toString()}
                alt="ItemImage"
              ></img>
            ) : (
              <l className="iconImage">
    ...
    <div className="view_list">
      <div className="view_header">
        ...
        <h4>
          ...
          Sort by:
          <span />{" "}
          <button className="sort-button" id="name" onClick={sortDataHandle}>
            Name{" "}
          </button>
          <span />{" "}
          <button className="sort-button" id="price" onClick={sortDataHandle}>
            Price
          </button>
        </h4>
      </div>
      <div className="view_display">
        ...
        <br />
        <br />
        <button className="add-button" id="addItem" onClick={addNewItem}>
          {" "}
          Add Item{" "}
        </button>
      </div>
